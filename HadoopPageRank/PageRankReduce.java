...
public class PageRankReduce extends Reducer<LongWritable, Text, LongWritable, Text>{
	public void reduce(LongWritable key, Iterable<Text> values,
		...
		for (Text value: values){
			...
			if (strArray.length == 1)//No out urls 
			{
				sumOfRankValues += Double.parseDouble(strArray[0]); //rank this url
			}
			else // If there are out urls
			{   
		...
		//Emit key value output //Final output
